...
function listarEstados() {
    ...
    fetch("/planos/listarEstados", { cache: 'no-store' })
...
function listarMunicipiosSelect() {
    ...
    `;
    fetch("/planos/listarMunicipios", { cache: 'no-store' })
    .then(function (response) {
        if (response.ok) {
            response.json().then(function (resposta) {
                console.log(resposta)
...
function listarEventosSelect() {
    ...
    `;
    fetch("/planos/listarEventos", { cache: 'no-store' })
    .then(function (response) {
        if (response.ok) {
            response.json().then(function (resposta) {
                console.log(resposta)
...
function renderizarCardsAprovados() {
  ...
  pagina.forEach((plano) => {
    ...
    card.innerHTML = `
        <h3>${plano.plano}</h3>
        <div class="info-item">
            <div class="icon-circle"><i class="ti ti-map-pin" style="color: #008000"></i></div>
            <p><strong>Destino:</strong><br>${plano.destino} - ${plano.uf}</p>
        </div>
        <div class="info-item">
            <div class="icon-circle"><i class="ti ti-clock-hour-11" style="color: #008000"></i>
            </div>
            <p><strong>Duração:</strong><br>${plano.duracao} dias</p>
        </div>
        <div class="info-item no-border">
            <div class="icon-circle"><i class="ti ti-calendar-month" style="color: #008000"></i></div>
...
function renderizarPaginacaoAprovados() {
    ...
    for (let i = 1; i <= totalPaginas; i++) {
      ...
      btn.className = 'paginacao-btn' + (i === paginaAtualAprovados ? ' ativo' : '');
      btn.textContent = i;
      btn.onclick = () => {
...
function renderizarPaginacaoPendentes() {
    ...
    for (let i = 1; i <= totalPaginas; i++) {
      const btn = document.createElement('button');
      btn.className = 'paginacao-btn' + (i === paginaAtualPendentes ? ' ativo' : '');
      btn.textContent = i;
      btn.onclick = () => {
...
function mudarPaginaAprovados(direcao) {
    const totalPaginas = Math.ceil(planosAprovados.length / ITENS_POR_PAGINA);
...
function mudarPaginaPendentes(direcao) {
    const totalPaginas = Math.ceil(planosPendentes.length / ITENS_POR_PAGINA);
